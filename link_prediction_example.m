...
alpha0 = .5;
for j = 1 : numtrials
    ind_deleted_edges = randi([1,m],floor(tau*m),1); %remove random edges, will it sill be connected
    
    H = G.rmedge(ind_deleted_edges);
    A = H.adjacency();
    T = build_triangles_tensor(A,'type','random_walk');
    D = 1./sum(A,2); %inverse of degree of nodes
    D(D == inf) = 0; %if degree(i) = 0, Di=inf
    D = spdiags(D,0,n,n);
    M = D*A;
